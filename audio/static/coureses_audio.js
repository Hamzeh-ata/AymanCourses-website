...
const audioTitle = document.getElementById('plyer-title');
const title=document.getElementById('title');
var myAudio = document.getElementById('my-audio');
var seeking = false;
var seekto = 0;
/*rightButton.onclick=function(){
    document.getElementById('section-2').scrollLeft +=90;
}
...
const ap_html = '<style>.ap_container{transition-duration:.5s;display:inline-block;text-align:center;overflow:hidden;padding:5px;background:rgba(245,236,242);border-radius:4px;transition:all 1s}.ap_controls{width:100%;display:flex;position:relative;flex-direction:row;font-size:large;font-family:Tahoma,Geneva,Verdana,sans-serif;cursor:pointer}.ap_controls span{margin:5px;display:inline-block}.ap_container .playpausebtn{border-radius:50%;font-size:larger}.ap_container svg{width:25px;height:25px}.ap_controls .duration{width:80%;margin-right:10px;bottom:4px}.ap_controls input[type=range]{width:100%;outline:0;height:2px;margin:0 15px;background:#25262a}.ap_controls:hover input[type=range]::-webkit-slider-thumb{-webkit-appearance:none;height:20px;width:20px;border-radius:50%;cursor:pointer;background:#00ced1}.ap_controls input[type=range]::-webkit-slider-thumb{-webkit-appearance:none;height:0}.ap_sound{padding:10px}.ap_time{margin:4px   color:white}</style><div class="ap_container"> <div class="ap_controls"> <span class="playpausebtn" onclick="justplay()"> </span> <span class="duration"> <input type="range" min="0" max="100" value="0" class="duration_slider" onchange="change_duration()"> </span> <span class="ap_time"></span> <span class="ap_download" onclick="ap_download()"></span> <span class="ap_sound" onclick="ap_mute()"> </span> </div> </div>';
//Constants
...
else if(isNaN(ap_audio.duration)){
o('.ap_time').innerHTML = '0:00';
}
...
function range_slider() {
    ...
    if (track.ended) {
      play.style.background = 'url(./imgs/audio_playIcon.png)  no-repeat'
      if (autoplay == 1) {
            index_no += 1;
            //load_track(index_no);
            playsong();
...
function ap_mute() {
    if (!ap_audio.muted) {
        ap_audio.muted = 1;
        o('.ap_sound').style.background  = 'url(/static/imgs/audio_muteicon.png)  no-repeat'
    } else {
        ap_audio.muted = 0;
        o('.ap_sound').style.background = 'url(/static/imgs/audio_unmuteicon.png)  no-repeat'
    }
}
...
o('.playpausebtn').style.background = 'url(/static/imgs/audio_playIcon.png)  no-repeat'
o('.ap_sound').style.background = 'url(/static/imgs/audio_unmuteicon.png)  no-repeat'
// play song
function playsong() {
    ...
    track.play();
    Playing_song = true;
    play.style.background = 'url(/static/imgs/audio_pauseIcon.png)  no-repeat'

}http://127.0.0.1:8000/home/audio/audio/static/imgs/audio_playIcon.png
//pause song
function pausesong() {
    track.pause();
    Playing_song = false;     

    play.style.background = 'url(/static/imgs/audio_playIcon.png)  no-repeat'
  }